...
def reset_app():
    # Preserve the keys for company info.
    keys_to_preserve = {"client_info", "your_info"}
...
st.set_page_config(layout="wide", page_title="Five-Step Proposal Builder")
load_state()
...
def section_2():
    ...
    # Identify Industry based on aggregated client Company Info
    client_info_text = st.session_state["sources"].get("client_Company Info", "")
    if client_info_text:
        st.session_state["identified_industry"] = identify_industry(client_info_text)
    else:
        st.session_state["identified_industry"] = "Unknown"
...
# Compute common variables for Step 3 from stored state
...
# Prepare company details using proposal_settings from Step 2
company_name = st.session_state.get("company_name", "").strip() or "Unknown Company"
...
tone = st.session_state["proposal_settings"].get("tone", "Formal")  # UPDATE: unified tone
...
def section_3():
    ...
    with col_left:
        ...
        with section_nav[1]:
            # Recompute sections_list for consistency
        ...
        # Recompute sections_list and current_section
        outline_text = st.session_state.get("proposal_sections", "")
        sections_list = [sec.lstrip("- ").strip() for sec in outline_text.splitlines() if sec.strip()]
        ...
        # UPDATE: Build prompt with aggregated sources and ranked needs
        base_prompt = f"""
...
"""
        ...
        if "Technical Approach" in current_section:
            additional_guidance = """
In this Technical Approach, please:
1. Reference the Statement of Work (SOW) or RFP sections explicitly (e.g., "Per Section 2.2 of the SOW...") to show alignment with the client’s requirements.
2. Provide a phased or structured approach (e.g., Phase 1: Requirements, Phase 2: Development, etc.).
3. Mention specific compliance/security standards (e.g., FISMA, NIST, or other relevant frameworks).
4. Highlight how the solution will be integrated and how performance/metrics will be tracked.
5. Incorporate any best practices for Government RFP responses, including stakeholder collaboration.
"""
...
"""
        ...
        if st.button("Generate FocusGPT Expert Suggestions"):
            focusgpt_prompt = f"""
You are FocusGPT, an expert proposal writer. Provide 3 to 4 concise bullet points that offer expert suggestions to improve the following section content.
Section Content:
{edited_draft}
"""
            focus_suggestions = generate_summary("", focusgpt_prompt)
            st.session_state["focus_suggestions"] = focus_suggestions
            st.write(focus_suggestions)
        if "focus_suggestions" in st.session_state:
            if st.button("Apply FocusGPT Suggestions"):
                apply_prompt = f"""
You are FocusGPT, an expert proposal writer. Revise the following section content by incorporating the expert suggestions provided below. Produce a revised version that improves clarity, structure, and compliance.
Expert Suggestions:
{st.session_state['focus_suggestions']}
...
def section_4():
    ...
    with col_left:
        ...
        file_format = st.selectbox("Select Export Format", ["DOCX", "PDF", "HTML"], index=0)
        ...
        metadata = st.text_area("Enter metadata (submission date, contact info, version):", value=st.session_state.get("metadata", ""))
        st.session_state["metadata"] = metadata
    with col_right:
        st.header("Final Proposal Preview")
        final_doc = Document()
        ...
        st.download_button("Download Final Proposal (DOCX)", open(temp_file.name, "rb"),
...
# =============================================================================
def main():
    st.title("Five-Step Proposal Builder")
